...
if ( class_exists( 'OT_Loader' ) && defined( 'OT_PLUGIN_MODE' ) && OT_PLUGIN_MODE == true ) {
  ...
  function ot_conflict_notice() {
    ...
    echo '<div class="error"><p>' . __( 'OptionTree is installed as a plugin and also embedded in your current theme. Please deactivate the plugin to load the theme dependent version of OptionTree, and remove this warning.', 'parada' ) . '</p></div>';
...
if ( ! class_exists( 'OT_Loader' ) ) {
  ...
  class OT_Loader {
    ...
    private function load_languages() {
       ...
       * @since     2.1.3
       */
      define( 'OT_PLUGIN_MODE', strpos( get_template_directory() , 'plugins' . DIRECTORY_SEPARATOR . basename( get_template_directory()  ) ) !== false ? true : false );
      ...
      if ( OT_PLUGIN_MODE ) {
        ...
        if ( apply_filters( 'ot_child_theme_mode', false ) == true ) {
          ...
          $path = @explode( get_stylesheet(), str_replace( '\\', '/', get_template_directory()  ) );
          $path = ltrim( end( $path ), '/' );
          define( 'OT_LANG_DIR', trailingslashit( trailingslashit( get_stylesheet_directory() ) . $path ) . trailingslashit( 'languages' ) . 'theme-mode' );
          
        } else {
          
          $path = @explode( get_template(), str_replace( '\\', '/', get_template_directory()  ) );
          $path = ltrim( end( $path ), '/' );
    ...
    public function load_textdomain() {
      ...
      if ( OT_PLUGIN_MODE ) {
        ...
        load_plugin_textdomain( 'parada', false, OT_LANG_DIR );
        
      } else {
      
        load_theme_textdomain( 'parada', OT_LANG_DIR );
    ...
    private function constants() {
      ...
      if ( false == OT_THEME_MODE && false == OT_CHILD_THEME_MODE ) {
        ...
        if ( true == OT_CHILD_THEME_MODE ) {
          $path = ltrim( end( @explode( get_stylesheet(), str_replace( '\\', '/', get_template_directory()  ) ) ), '/' );
    ...
    public function change_image_button( $translation, $text, $domain ) {
      ...
      if ( $pagenow == apply_filters( 'ot_theme_options_parent_slug', 'themes.php' ) && 'default' == $domain && 'Insert into post' == $text ) {
        ...
        return apply_filters( 'ot_upload_text', __( 'Send to OptionTree', 'parada' ) );
